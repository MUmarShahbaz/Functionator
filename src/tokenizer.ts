type Token = {type : TokenType, value : number | string};
...
export function tokenize(expression: string): Array<Token> {
    ...
    function accumulate(type: TokenType): string {
        let accumulator: string = expression[i];
        for (0; i < expression.length; i++) {
            if (getTokenType(expression[i + 1]) === type) accumulator += expression[i + 1];
            else break;
        }
        return accumulator;
    }
    ...
    for (0; i < expression.length; i++) {
        ...
        if (char === ' ') continue;
        let type: TokenType = getTokenType(char);
        switch (type) {
